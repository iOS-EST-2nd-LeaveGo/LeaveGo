...
extension NetworkManager {
    ...
    func fetchPlaceList(page: Int = 1, numOfRows: Int = 100, mapX: Double, mapY: Double, radius: Int, contentTypeId: Int?, arrange: String = "E") async throws -> (places: [PlaceList], totalCount: Int) {
        // 장소 목록을 담을 변수 선언
        var placeList = [PlaceList]()
        var totalCount = 0

        // endpoint 에 필수값들을 전달해 URL 생성
        let endpoint = Endpoint.placeList(page: page, numOfRows: numOfRows, mapX: mapX, mapY: mapY, radius: radius, contentTypeId: contentTypeId, arrange: arrange)
